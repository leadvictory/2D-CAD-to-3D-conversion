...
class CadCodeGeneratorChain(SequentialChain):
    ...
    def __init__(self, model_type: MODEL_TYPE = "gpt") -> None:
        ...
        gen_cad_code_prompt = (
            ...
            "## Points to Note\n"
            "* must read all geometry values and its relationship from uploaded 2D CAD image and check if all of them used to generate 3D CAD model at final code"
            ...
            "* Ensure that the generated CadQuery code is syntactically correct and executable.\n"
            "* Pay close attention to the order of operations and the correct usage of CadQuery functions.\n"
            "* Before generating the code, double-check that it will produce a valid 3D model.\n"
            "* Use the `Workplane` object directly for creating and modifying the 3D model. Avoid using the `Sketch` object.\n"
            "* Ensure that the generated CadQuery code produces a valid 3D model that can be exported as a STEP file without errors.\n"
            "* Double-check the geometry and operations to avoid creating invalid or corrupted STEP files.\n"
            "* It is crucial to use the template string `{{output_filename}}` when specifying the output file path. The output file name must be exactly 'output.step'.\n"
